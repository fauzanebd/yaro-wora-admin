...
import {
  regulationContentFormSchema,
  type RegulationContentForm,
} from "@/types/regulation-content-form";
...
interface RegulationContentDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  content: RegulationPageContent | null;
  onSubmit: (data: RegulationContentForm) => Promise<void>;
  trigger?: React.ReactNode;
}

export default function RegulationContentDialog({
  open,
  onOpenChange,
  content,
  onSubmit,
  trigger,
}: RegulationContentDialogProps) {
  const uploadFile = useUploadFile();

  const form = useForm<RegulationContentForm>({
    resolver: zodResolver(regulationContentFormSchema),
    defaultValues: {
  ...
  const handleSubmit = async (data: RegulationContentForm) => {
    await onSubmit(data);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Edit Regulations Page Content</DialogTitle>
          <DialogDescription>
            Update header and CTA sections for the regulations page
          </DialogDescription>
        </DialogHeader>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="e.g., Village Regulations"
                            {...field}
                          />
                        </FormControl>
                        <FormMessage />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="e.g., Peraturan Desa"
                            {...field}
                          />
                        </FormControl>
                        <FormMessage />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          Main hero image for the regulations page
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
